...
> **Ai được vào mạng, vào tới đâu, và làm sao biết ai đã làm gì?**

Đó là ba chữ A: **Authentication** (anh là ai) · **Authorization** (được làm gì) ·
**Accounting** (đã làm gì).

## 🔴 Bẫy phân bổ thời gian — Domain 5.0 chia đôi rõ rệt

```
   ┌──────────────────────────────────────────────────────────┐
   │  PHẢI GÕ ĐƯỢC   ("Configure and verify")                 │
   │    §3 Hardening · §4 AAA · §5 ACL · §6 CoPP · §8 Wireless│
   ├──────────────────────────────────────────────────────────┤
   │  CHỈ CẦN NÓI ĐƯỢC   ("Describe")                         │
   │    §9 TrustSec/MACsec · §10 NGFW · §11 REST API          │
   └──────────────────────────────────────────────────────────┘

   Sai lầm điển hình: đọc cả tuần về Firepower / Umbrella / Zero Trust
   (rất hấp dẫn, nhưng chỉ "describe") rồi vào phòng thi
   KHÔNG CẤU HÌNH NỔI một method list AAA có fallback.
```

## Bảng TACACS+ vs RADIUS — đề hỏi nhiều nhất module

| | ⭐⭐ **TACACS+** | ⭐⭐ **RADIUS** |
|---|---|---|
| Giao thức / port | ⭐ **TCP 49** | ⭐ **UDP 1812** (auth) / **1813** (acct) |
| Mã hóa | ⭐⭐ **TOÀN BỘ phần thân gói** | 🔴 **CHỈ trường password** |
| Tách 3 chữ A | ⭐ **Tách cả ba** | 🔴 Gộp authn + authz |
| Command authorization | ⭐⭐ **CÓ** — cho/cấm **từng lệnh** | ❌ Không |
| ⭐ **Dùng cho** | ⭐⭐ **Quản trị THIẾT BỊ** | ⭐⭐ **Cho người dùng vào MẠNG** |

> ⭐ **Mẹo nhớ:** **T**ACACS+ = **T**CP = **49** · quản trị **T**hiết bị.

## 8 ý phải nhớ

| # | Ý | Một câu |
|:---:|---|---|
| 1 | 🔴 ⭐⭐ **`reject` vs `timeout`** | Server trả lời *"sai"* = **đã trả lời** → **KHÔNG fallback**. Chỉ khi server **im lặng** mới dùng `local` |
| 2 | 🔴 ⭐⭐ **4 quy tắc AAA sống còn** | Tạo user local **TRƯỚC** `aaa new-model` · luôn có **`local`** cuối list · **giữ phiên SSH thứ hai** · **`test aaa`** trước khi logout |
| 3 | 🔴 ⭐⭐ **Type 7 KHÔNG phải bảo mật** | `service password-encryption` chỉ tạo type 7 — **giải ngược trong 1 giây**. Dùng **type 9 (scrypt)** |
| 4 | ⭐ **ACL: Standard gần ĐÍCH, Extended gần NGUỒN** | Và luôn viết **`deny ip any any log`** tường minh *(deny ngầm không đếm, không log)* |
| 5 | 🔴 ⭐⭐ **Bẫy IPv6 ACL** | Viết `deny ipv6 any any` tường minh → **GIẾT NDP** → IPv6 chết sạch |
| 6 | ⭐⭐ **CoPP bảo vệ CONTROL PLANE** | Không phải data plane. ⭐ **Đo trước, siết sau** — siết ngay là tự sập OSPF/SSH |
| 7 | ⭐⭐ **802.1X: switch chỉ là người đưa thư** | **SERVER ra quyết định.** EAPoL ở **Layer 2** — client **chưa có IP** |
| 8 | 🔴 ⭐⭐ **401 vs 403** | **401** = chưa xác thực *(xin token mới)* · **403** = đã xác thực nhưng **không có quyền** *(xin token vô ích)* |

## Bảng lệnh cốt lõi

| Lệnh | Cho biết gì / làm gì |
|---|---|
| ⭐⭐ `test aaa group <G> <user> <pass> legacy` | **Test AAA TRƯỚC KHI LOGOUT** — lệnh cứu mạng |
| ⭐ `show aaa servers` | Server UP/DEAD + **`reject` hay `timeouts`** |
| ⭐⭐ `show access-lists <NAME>` | **Bộ đếm từng dòng** — kỹ thuật troubleshoot ACL số 1 |
| `clear ip access-list counters <NAME>` | Reset trước khi test |
| ⭐ `show policy-map control-plane` | CoPP — `conformed` / `exceeded` |
| ⭐⭐ `show dot1x \| include Sysauthcontrol` | **Kiểm tra ĐẦU TIÊN** khi 802.1X "không chạy" |
| ⭐⭐ `show access-session interface <x> details` | **Client dừng ở State nào** |
...
| **2** | ⚙️ **NÓ CHẠY THẾ NÀO** — ⭐ **§4 AAA là trọng tâm số 1** | 6 giờ |
| **3** | 🧪 **NHÌN THẤY NÓ** — [LAB 10](Module-10-LAB.md), ⭐ **không cần RADIUS vẫn lab được 80%** | 7 giờ |
| **4** | 🏗️ **TOPO & KIẾN TRÚC** — an ninh theo LỚP | 45 phút |
| **📎** | **PHỤ LỤC** — 🔴 không đọc lần đầu | — |

> ⭐ **Ba lab "cố ý phá" đáng làm nhất:**
> 1. **Giải ngược chuỗi password type 7** bằng công cụ online — 2 phút, nhớ cả đời
...
>
> Security là khối **nhiều thuật ngữ nhất** của ENCOR (AAA, CoPP, 802.1X, TrustSec, MACsec…).
> Sáu ví von dưới đây là cách nhanh nhất để chúng thôi là từ viết tắt.
>
> **Tự kiểm tra:** đọc xong mỗi mục, gấp tài liệu lại, nói lại trong 3 câu.

### 2.1 AAA là "thẻ nhân viên, quyền mở cửa, và sổ ra vào"
...
### 2.2 Fallback: vì sao "sai mật khẩu" khác "server chết"
...
### 2.3 CoPP: bảo vệ "bộ não", không phải "đôi chân"
...
### 2.4 802.1X là "bảo vệ ở cửa, KHÔNG phải người quyết định"
...
### 2.5 TrustSec: dán nhãn người, đừng đánh số ghế
...
### 2.6 MACsec vs IPsec: xe bọc thép từng chặng vs vali khóa suốt hành trình
...
> |---|:---:|---|
> | §2.1 thẻ nhân viên · §2.2 nhân sự không bắt máy | → | **§4 AAA** 🔴 ⭐⭐ *(trọng tâm số 1)* |
> | §2.3 thư ký gác cửa bộ não | → | **§6 CoPP** 🔴 ⭐⭐ |
> | §2.4 bảo vệ giữ cửa nhưng không có quyền | → | **§7 802.1X / MAB / WebAuth** |
> | §2.5 dán nhãn lên NGƯỜI thay vì đánh số GHẾ | → | **§9 TrustSec** |
> | §2.6 xe bọc thép vs vali khóa | → | **§9 MACsec vs IPsec** |
>
> 🔴 ⭐ **Domain 5.0 chia đôi rõ rệt — đây là bẫy phân bổ thời gian:**
>
> | Mục | Blueprint | Nghĩa |
> |---|---|---|
> | ⭐⭐ **§3 hardening · §4 AAA · §5 ACL · §6 CoPP · §8 wireless** | 🔴 **"Configure and verify"** | **PHẢI GÕ ĐƯỢC** |
> | 🟡 **§9 TrustSec/MACsec · §10 NGFW · §11 REST API** | 🟡 **"Describe"** | **Chỉ cần nói được** |
>
> Sai lầm điển hình: đọc cả tuần về Firepower/Umbrella/Zero Trust (chỉ *describe*, rất hấp dẫn)
> rồi **không cấu hình nổi một method list AAA có fallback**.

---

## 📘 3. DEVICE ACCESS CONTROL (blueprint 5.1.a — CONFIGURE)

### 3.1 ⭐ Ba đường vào thiết bị
...
### 3.2 ⭐⭐ Các loại mật khẩu — bảng phải nhớ
...
### 3.3 ⭐ Privilege level & Role-Based CLI
...
### 3.5 ⭐ Checklist hardening — bảng dùng được ngoài đời
...
## 📘 4. 🔴 ⭐⭐ AAA (blueprint 5.1.b — CONFIGURE) — trọng tâm số 1 của module
...
### 4.2 ⭐⭐ TACACS+ vs RADIUS — bảng ĐỀ HỎI NHIỀU NHẤT
...
### 4.5 🔴 ⭐⭐ CẢNH BÁO: cách tự khóa mình ra khỏi thiết bị
...
### 5.2 ⭐⭐ Named ACL & sequence number — sửa ACL mà không phải xóa hết
...
### 5.4 🔴 ⭐⭐ IPv6 ACL — BẪY LỚN NHẤT của mục 5.2.a
...
### 5.5 ⭐⭐ PACL / VACL / RACL — ba loại ACL trên switch
...
### 5.6 ⭐ uRPF — chống giả mạo địa chỉ nguồn
...
## 📘 6. 🔴 ⭐⭐ CoPP (blueprint 5.2.b — CONFIGURE)

### 6.1 ⭐ Vấn đề CoPP giải quyết
...
### 6.3 ⭐ CPPr — bản nâng cấp của CoPP
...
## 📘 7. ⭐⭐ NETWORK ACCESS CONTROL — 802.1X / MAB / WebAuth (blueprint 5.5.e)
...
### 7.3 ⭐⭐ MAB & WebAuth — hai phương án dự phòng
...
### 7.5 ⭐⭐ Cấu hình 802.1X trên switch
...
### 7.6 ⭐⭐ Ba VLAN dự phòng — và chế độ triển khai
...
## 📘 8. 🔴 ⭐⭐ WIRELESS SECURITY (blueprint 5.4 — CONFIGURE)
...
### 8.2 ⭐ Cấu hình PSK / SAE (5.4.c) trên Catalyst 9800
...
### 8.3 ⭐⭐ Cấu hình EAP / 802.1X wireless (5.4.a)
...
## 📘 9. ⭐⭐ TRUSTSEC & MACSEC (blueprint 5.5.d — Describe)

### 9.1 ⭐ Vấn đề TrustSec giải quyết
...
### 9.2 ⭐⭐ Ba pha của TrustSec — bảng phải thuộc
...
## 📘 10. 🟡 THÀNH PHẦN THIẾT KẾ AN NINH MẠNG (blueprint 5.5.a/b/c — Describe)

> ⭐ Mục này ⭐ **chỉ cần "describe"** — ⭐ **học bảng, biết tên, hiểu vai trò. KHÔNG cấu hình.**

### 10.1 ⭐⭐ NGFW vs Firewall truyền thống vs UTM
...
### 10.3 ⭐ Threat defense — hệ sinh thái Cisco
...
### 10.5 ⭐ Zero Trust & Defense in Depth
...
## 📘 11. 🟡 REST API SECURITY (blueprint 5.3 — Describe)

> ⭐ Mục nhỏ nhưng ⭐ **chắc chắn có câu hỏi**, và nó là ⭐ **nền cho Module-12**.

### 11.1 ⭐⭐ Các cách xác thực API
...
### 11.2 ⭐⭐ Luồng token của hai sản phẩm Cisco — đề hay hỏi
...
### 11.3 ⭐⭐ Mã trạng thái HTTP — 401 vs 403 là câu kinh điển
...
### 11.4 ⭐ Checklist bảo mật REST API
...
## 📘 12. 🟡 BỔ TRỢ — Tính năng an ninh Layer 2
...
> Bạn vừa học 10 công nghệ an ninh rời rạc. Phần này xếp chúng vào **đúng chỗ trong mạng**.

### 4.1 Bản đồ: an ninh theo LỚP, không phải theo tính năng

```
                        INTERNET
                            │
                  ┌─────────┴──────────┐
   BIÊN           │  ⑤ NGFW / FTD      │   nhận diện ỨNG DỤNG + NGƯỜI DÙNG
                  │     + IPS inline    │   (firewall thường chỉ thấy "TCP 443")
                  └─────────┬──────────┘
                            │
                  ┌─────────┴──────────┐
   LÕI / DIST     │  ③ ACL (policy)    │   ⭐ ACL đặt ở ĐÂY, không phải core
                  │  ④ CoPP            │   bảo vệ CPU khỏi bị làm ngập
                  │  ① AAA (TACACS+)   │   ai được SSH vào và gõ lệnh gì
                  └─────────┬──────────┘
                            │
                  ┌─────────┴──────────┐
   ACCESS         │  ② 802.1X / MAB    │   ⭐ chặn NGAY TỪ TRƯỚC khi có IP
                  │  ⑥ TrustSec (SGT)  │   ISE gán nhãn theo DANH TÍNH
                  │  ⑦ MACsec (uplink) │   mã hóa L2 từng chặng
                  └─────────┬──────────┘
                            │
                    [PC] [Phone] [Camera]
```

### 4.2 Bảng: dùng giao thức nào cho việc gì

| Việc | Dùng | Vì sao |
|---|---|---|
| ⭐ **Ai được SSH vào thiết bị và gõ lệnh gì** | ⭐⭐ **TACACS+** *(TCP 49)* | Mã hóa **toàn bộ gói** · tách cả 3 chữ A · ⭐ **có command authorization** |
| ⭐ **Ai được vào mạng** (802.1X, Wi-Fi, VPN) | ⭐⭐ **RADIUS** *(UDP 1812/1813)* | Chuẩn mở · gộp authn+authz · 🔴 chỉ mã hóa **trường password** |
| Chặn thiết bị lạ ngay tại cổng | **802.1X** *(có supplicant)* · **MAB** *(máy in, camera)* | ⭐ **EAPoL ở Layer 2 — chặn TRƯỚC khi có IP** |
| Chính sách không phụ thuộc IP | ⭐ **TrustSec / SGT** | Gán nhãn theo **danh tính**, đổi subnet không phải viết lại ACL |
| Mã hóa đường trục trong nhà mình | ⭐ **MACsec** *(L2, hop-by-hop)* | Line-rate bằng phần cứng |
| Mã hóa qua Internet | ⭐ **IPsec** *(L3, end-to-end)* | Không tin ai ở giữa |

### 4.3 Bốn quy tắc sống còn khi triển khai

| # | Quy tắc | 🔴 Không làm thì sao |
|:---:|---|---|
| ① | ⭐⭐ **Tạo user local TRƯỚC `aaa new-model`, và luôn có `local` cuối method list** | **Tự khóa mình ra khỏi thiết bị** — phải đi tới tận nơi cắm console |
| ② | ⭐⭐ **CoPP: đo trước, siết sau** *(`exceed-action transmit` trước)* | Siết ngay = **tự đánh sập OSPF/SSH của chính mình** |
| ③ | ⭐⭐ **802.1X: Monitor → Low-Impact → Closed** | Bật thẳng Closed = **hàng chục máy in/camera không ai biết rớt cùng lúc** |
| ④ | ⭐⭐ **Cấu hình Critical VLAN trước khi bật 802.1X** | ISE bảo trì 10 phút = **cả công ty mất mạng**, và bạn cũng không SSH vào được |

### 4.4 Ba sự thật mà chỉ người đi làm mới biết

| Sự thật | Giải thích |
|---|---|
| 🔴 ⭐⭐ **`reject` KHÔNG fallback, `timeout` MỚI fallback** | Server trả lời *"sai mật khẩu"* = **đã trả lời** → dừng lại. Chỉ khi server **im lặng** mới chuyển sang `local`. ⭐ **Và sai shared key trông y hệt "server chết"** |
| 🔴 ⭐⭐ **Type 7 KHÔNG phải bảo mật** | `service password-encryption` chỉ tạo type 7 — **giải ngược trong 1 giây**. Phải dùng `algorithm-type scrypt` *(type 9)* |
| 🔴 ⭐⭐ **Viết `deny ipv6 any any` tường minh sẽ GIẾT NDP** | IPv6 ACL có **3 dòng ngầm**, trong đó 2 dòng cho phép NDP nằm **trước** dòng deny. Viết deny tường minh đẩy chúng ra sau → **IPv6 chết hoàn toàn** |

### 4.5 Những thứ này sẽ lớn lên thành gì

| Bạn vừa học | Sẽ thành | Ở module |
|---|---|---|
| ⭐ **TrustSec / SGT** | Chính là **policy plane của SD-Access** *(đã gặp ở M09 §8)* | — |
| CoPP | Hiểu **punt** và **control plane** *(đã học ở M01 §3.1)* | — |
| AAA, syslog của sự kiện an ninh | Syslog tập trung · SNMP · **NTP là nền của cả hai** | **Module-11** |
| REST API security *(401 vs 403)* | Gọi API DNAC/vManage bằng Python | **Module-12** |
...
> 2. Ghi rõ: chỗ nào dùng **TACACS+**, chỗ nào dùng **RADIUS**, và **vì sao**
> 3. Trả lời: *Bạn cấu hình AAA với TACACS+, gõ sai mật khẩu và mong router chuyển sang dùng user local. Nhưng nó từ chối luôn. Vì sao?*

<details>
<summary>Đáp án câu 3</summary>

⭐⭐ Vì **server ĐÃ TRẢ LỜI** — nó trả về `reject` (*"sai mật khẩu"*).

Method list **chỉ chuyển sang phương pháp tiếp theo khi phương pháp trước KHÔNG TRẢ LỜI**.
Server trả lời *"không"* cũng là **đã trả lời** → router dừng lại, từ chối đăng nhập.
...
🔴 **Và một bẫy đi kèm:** nếu bạn **sai shared key**, router không giải mã nổi câu trả lời
→ biểu hiện **giống hệt "server chết"** → nó sẽ fallback. Dễ chẩn đoán nhầm.
...
> | Khi nào | Mở mục nào |
> |---|---|
> | Đang lab mà lỗi | **Gỡ lỗi nhanh** (§18) — ⭐ quy trình chẩn đoán 802.1X 6 bước |
